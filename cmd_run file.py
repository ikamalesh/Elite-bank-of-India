    ...
    def after_login(dictionary):
        while True:
            ...
            if user_input == 1:
                ...
                db.child('users').child(to_email).child('transactions').push(
                    f"{now} {ref_code} {f'Rs.{amount} CREDIT':>17} - {f'Acc No.{acc_no}': <5}")

                db.child('users').child(email_dot).update({'balance': sender_amt})
                db.child('users').child(email_dot).child('transactions').push(
                    f"{now} {ref_code} {f'Rs.{amount} DEBIT':>17} - {f'Acc No.{to}': <5}")
